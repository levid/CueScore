// Generated by CoffeeScript 1.3.1
(function() {

  describe("NineBall Rank", function() {
    var model;
    model = void 0;
    beforeEach(function() {
      return model = new $CS.Models.NineBall.Ranks();
    });
    return describe("Scoring", function() {
      it("should return the correct Ball Counts for each rank", function() {
        expect(model.getBallCount(1)).toEqual(14);
        expect(model.getBallCount(2)).toEqual(19);
        expect(model.getBallCount(3)).toEqual(25);
        expect(model.getBallCount(4)).toEqual(31);
        expect(model.getBallCount(5)).toEqual(38);
        expect(model.getBallCount(6)).toEqual(46);
        expect(model.getBallCount(7)).toEqual(55);
        expect(model.getBallCount(8)).toEqual(65);
        return expect(model.getBallCount(9)).toEqual(75);
      });
      it("should be able to return the match points for the losing player", function() {
        return expect(model.getLosingPlayersMatchPoints(1, 12)).toEqual(8);
      });
      it("should be able to return the match points for the winning player", function() {
        return expect(model.getWinningPlayersMatchPoints(1, 2)).toEqual(20);
      });
      return it("should be able to return the number of timeouts based on model", function() {
        expect(model.getTimeouts(1)).toEqual(2);
        expect(model.getTimeouts(2)).toEqual(2);
        expect(model.getTimeouts(3)).toEqual(2);
        expect(model.getTimeouts(4)).toEqual(1);
        expect(model.getTimeouts(5)).toEqual(1);
        expect(model.getTimeouts(6)).toEqual(1);
        expect(model.getTimeouts(7)).toEqual(1);
        expect(model.getTimeouts(8)).toEqual(1);
        expect(model.getTimeouts(9)).toEqual(1);
        return expect(model.getTimeouts(20)).toEqual(0);
      });
    });
  });

}).call(this);
